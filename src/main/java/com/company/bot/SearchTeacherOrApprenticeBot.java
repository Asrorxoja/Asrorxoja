...
public class SearchTeacherOrApprenticeBot extends TelegramLongPollingBot {
    @Override
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage()) {
            ...
            Message messageCall = callbackQuery.getMessage();
            User user = callbackQuery.getFrom();
            String data = callbackQuery.getData();

            String chatId = String.valueOf(messageCall.getChatId());

            if (chatId.equals(ComponentContainer.ADMIN_CHAT_ID)) {
                AdminController.handleCallback(user, messageCall, data);
            } else {
                MainController.handleCallback(user, messageCall, data);
            }
        }
    }
